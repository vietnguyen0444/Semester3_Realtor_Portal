...
namespace NETAPI_SEM3.Controllers
{
	...
	public class ImageController : ControllerBase
	{
		...
		public ImageController(IWebHostEnvironment _iwebHostEnvironment, NewsImageService _newsImageService, MemberService _memberService, SettingService _settingService, ImageService _imageService)
		{
		...
		public bool createPropertyImage(int propertyId, string fileName)
		{
			try
			{
				...
				var result = imageService.createImage(image);
				if (!result)
				{
					return false;
				}
		...
		public IActionResult DeleteImageByPropertyId(int id)
		{
			try
			{
				imageService.deleteImageByPropertyId(id);
				return Ok();
			}
			catch
			{
		...
		public IActionResult GetMaxPropertyImage()
		{
			try
			{
				return Ok(imageService.GetMaxNumberImageProperty());
			}
			catch
			{
				return BadRequest();
